...
package core.essentials.cache.impl
...
class MemoryCacheSource : ICacheSource {
    ...
    override suspend fun <T> get(key: ICacheKey<T>, valueProvider: suspend () -> T?): T? {
        val cacheKey = CacheKey(key)
        val cacheItem = cache[cacheKey]
    ...
    override suspend fun <T> put(key: ICacheKey<T>, value: T) {
        val cacheKey = CacheKey(key)
    ...
    override fun <K : ICacheKey<*>> invalidate(type: Class<K>) {
    ...
    override fun <K : ICacheKey<*>> invalidate(key: K) {
        val cacheKey = CacheKey(key)
        jobs.remove(cacheKey)?.cancel()
        cache[cacheKey]?.invalidate()
    }

    override fun <K : ICacheKey<*>> remove(type: Class<K>) {
        jobs.iterator().forEachRemaining { entry ->
            val job = entry.value
            val key = entry.key
            if (key.type == type) {
    ...
    override fun <K : ICacheKey<*>> remove(key: K) {
        val cacheKey = CacheKey(key)
        jobs.remove(cacheKey)?.cancel()
